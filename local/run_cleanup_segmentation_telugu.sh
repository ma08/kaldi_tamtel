...
stage=0
cleanup_stage=0
data=telugu_data/train
cleanup_affix=cleaned
srcdir=telugu_exp/tri3
nj=100
decode_nj=16
...
if [ $stage -le 1 ]; then
  ...
  steps/cleanup/clean_and_segment_data.sh --stage $cleanup_stage --nj $nj --cmd "$train_cmd" \
    $data telugu_data/lang $srcdir $dir $cleaned_data
  date
fi
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    $cleaned_data telugu_data/lang $srcdir ${srcdir}_ali_${cleanup_affix}
  date
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/train_sat.sh --cmd "$train_cmd" \
    5000 100000 $cleaned_data telugu_data/lang ${srcdir}_ali_${cleanup_affix} ${cleaned_dir}
  date
fi
...
if [ $stage -le 4 ]; then
  ...
  utils/mkgraph.sh telugu_data/lang ${cleaned_dir} ${cleaned_dir}/graph
